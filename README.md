# SOC Analyst Home Lab Portfolio

**Author:** Ibitayo Alasi ([@ialasi](https://github.com/ialasi))
**Current Role:** IT Support Specialist
**Target Role:** SOC Analyst
**Timeline:** 20 labs across 12 weeks

---

## About

This repository documents my hands-on SOC analyst training through 20 practical labs covering network analysis, SIEM, threat intelligence, incident response, and advanced detection techniques. Each lab is documented as a professional engineering report with evidence screenshots, commands used, and MITRE ATT&CK mappings.

---

## Lab Progress

| Lab | Title | Status | Week |
|---|---|---|---|
| 01 | Home SOC Lab Environment Setup | ✅ Complete | Week 1 |
| 02 | Wireshark Network Traffic Analysis | ✅ Complete | Week 1 |
| 03 | Linux Log Analysis + Syslog Parsing | ✅ Complete | Week 2 |
| 04 | Windows Event Log Investigation | ✅ Complete | Week 2 |
| 05 | Splunk Free SIEM Setup | ✅ Complete | Week 3 |
| 06 | Nmap Scanning + Alerting | ✅ Complete | Week 3 |
| 07 | Snort IDS + Custom Rules Writing | ✅ Complete | Week 4 |
| 08 | Phishing Email Analysis | ✅ Complete | Week 5 |
| 09 | Malware Sandbox Analysis | ✅ Complete | Week 5 |
| 10 | SIEM Brute Force Detection | ✅ Complete | Week 6 |
| 11 | Full incident response simulation — ransomware scenario | ✅ Complete | Week 7 |
| 12 | Threat intelligence with OpenCTI + MISP | 🔄 In Progress | Week 8, Day 1–4 |
| 13 | OSINT Threat Actor Attribution | ⏳ Pending | Week 8 |
| 14 | Vulnerability Scanning — OpenVAS | ⏳ Pending | Week 9 |
| 15 | Digital Forensics — Autopsy | ⏳ Pending | Week 10 |
| 16 | Active Directory Attack + Detection | ⏳ Pending | Week 10 |
| 17 | Endpoint Detection — Wazuh EDR | ⏳ Pending | Week 11 |
| 18 | AWS CloudTrail Log Analysis | ⏳ Pending | Week 11 |
| 19 | SOAR Automation — Shuffle | ⏳ Pending | Week 12 |
| 20 | Capstone — Full Portfolio Publish | ⏳ Pending | Week 12 |

---

## Tools Used

| Category | Tools |
|---|---|
| Virtualization | VirtualBox 7.2, Kali Linux, Windows 10, Docker, Kali Container |
| Network Analysis | Wireshark, tcpdump, Nmap |
| SIEM | Splunk Enterprise 10.4.1 |
| IDS/IPS | Snort |
| Threat Intel | MISP, OpenCTI, AlienVault OTX |
| OSINT | Shodan, URLScan, WHOIS |
| Forensics | Autopsy |
| EDR | Wazuh, Sysmon |
| Cloud | AWS CloudTrail, Athena |
| SOAR | Shuffle |

---

## Hardware

| Device | Specs |
|---|---|
| MacBook Pro M4 Pro | Apple Silicon, macOS | 24GB RAM
| HP Laptop | Ubuntu 24.04 LTS, 8GB RAM |
| Desktop Monitor | Ubuntu 22.04 LTS |

---

## Environment

- **Host OS:** Ubuntu 24.04 LTS
- **Hardware:** HP Laptop, 8GB RAM
- **Hypervisor:** Oracle VirtualBox 7.2
- **Lab Network:** Internal Network (192.168.56.0/24)

---

## Contact

- **GitHub:** [github.com/ialasi](https://github.com/ialasi)
- **Email:** ibitayoalasi@gmail.com

---

*Ibitayo Alasi — IT Support Specialist transitioning to SOC Analyst